...
#### 🌐 پشتیبانی از بیش از 212 سرور DNS معتبر
...
- **DNS Padding (RFC 8467)**: اضافه کردن 16-80 بایت Padding تصادفی برای جلوگیری از Traffic Analysis
- **QNAME Minimization**: حداقل‌سازی اطلاعات در Query ها
- **ECS Stripping**: حذف EDNS Client Subnet برای جلوگیری از نشت IP
...
- **Enhanced Decoy Requests** با احتمال 25% برای گمراه‌سازی
- **Request Deduplication** برای جلوگیری از Query های تکراری
...
#### 📊 مانیتورینگ و آمار
- **Real-time Stats Page** با Dark Theme
...
**فایل مورد نیاز:** `DoH_Proxy_Pro_Complete.txt`
...
3. محتوای فایل `DoH_Proxy_Pro_Complete.txt` را کپی کرده و جایگذاری کنید
...
└── README.md
```
3. فایل `[[path]].js` را در پوشه `functions/` قرار دهید
...
### DNS Padding چیست؟
تکنیکی مطابق RFC 8467 که 16-80 بایت داده تصادفی به Query اضافه می‌کند تا از Traffic Analysis و شناسایی الگوهای استفاده جلوگیری کند.

### QNAME Minimization چیست؟
تکنیکی برای حداقل کردن اطلاعات ارسالی در Query ها به منظور افزایش حریم خصوصی.

### ECS Stripping چیست؟
حذف EDNS Client Subnet از Query ها که از نشت اطلاعات IP شما به سرورهای DNS جلوگیری می‌کند.
...
### Request Deduplication چیست؟
جلوگیری از ارسال Query های تکراری در یک زمان. اگر دو درخواست یکسان همزمان بیاید، فقط یکی پردازش می‌شود.
